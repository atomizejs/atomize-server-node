...
(function () {
    ...
    Client.prototype = {
        ...
        bumpCreated: function (txnLog, txnSet) {
            ...
            for (i = 0; i < keys.length; i += 1) {
                ...
                globalTVar.lastTxnSet = txnSet;
                txnSet[globalTVar.id] = globalTVar.version;
            }
        },
        ...
        retry: function (txnLog) {
            ...
            this.bumpCreated(txnLog, txnSet);
        ...
        sendUpdates: function (updates) {
            ...
            while (0 < keys.length) {
                ...
                // Expand based on the txnSet
                if (undefined !== globalTVar.lastTxnSet) {
                    names = Object.keys(globalTVar.lastTxnSet); // names is global tvar ids
                    for (i = 0; i < names.length; i += 1) {
                        name = names[i]; // global tvar id
                        if (! util.hasOwnProp.call(updates, name)) {
                            j = this.toLocalTVarID(name);
                            if (undefined === j ||
                                this.localTVarVersion(j) < globalTVar.lastTxnSet[name]) {
                                keys.push(name);
                                updates[name] = true;
                            }
                        }
                ...
                if (undefined === i) {
                    i = this.addToMapping(globalTVar.id);
