...
(function () {
    ...
    TVar.prototype = {
        ...
        rollUpTxnSets: function () {
            ...
            if (this.txnSetLength > this.txnSetLengthLimit) {
                // Append first so that we can use this.lastTxnSet later on
                this.appendDependencies(dependencies);
                this.firstTxnSet = this.lastTxnSet;
                this.txnSetLength = 1;
                // Stop when we catch up with ourselves!
                while (dependencies !== txnSet.dependencies) {
    ...
    Client.prototype = {
        ...
        localTVarTxnSet: function (localTVarId) {
            return this.localGlobal[localTVarId].txnSet;
        },
        ...
        bumpCreated: function (txnLog, dependencies) {
            ...
            for (i = 0; i < keys.length; i += 1) {
                ...
                this.recordTVarVersion(keys[i], globalTVar);
            }
        },
        ...
        commit: function (txnLog) {
            ...
            if (ok && this.onPreCommit(txnLog)) {
                ...
                future.insert(dependencies, function () {
                    ...
                    for (i = 0; i < keys.length; i += 1) {
                        globalTVars[keys[i]].rollUpTxnSets();
                    }
                });
        ...
        sendUpdates: function (updates) {
            var empty = true,
                ...
                dependencies = {},
                key, globalTVar, localTVar, id, txnSet, names, name, i, desc, j;
            ...
            while (0 < keys.length) {
                ...
                id = this.toLocalTVarID(globalTVar.id);
                if (undefined === id) {
                    id = this.addToMapping(globalTVar.id);
                    txnLog.updates[id] = localTVar;
                    empty = false;
                } else {
                    if (this.localTVarVersion(id) === globalTVar.version) {
                ...
                txnSet = this.localTVarTxnSet(id);
                if (undefined === txnSet) {
                    txnSet = globalTVar.firstTxnSet;
                }
                while (undefined !== txnSet) {
                    names = Object.keys(txnSet.dependencies); // names is global tvar ids
                    for (i = 0; i < names.length; i += 1) {
                        name = names[i]; // global tvar id
                        if (! util.hasOwnProp.call(updates, name)) {
                            j = this.toLocalTVarID(name);
                            if (undefined === j ||
                                this.localTVarVersion(j) < txnSet.dependencies[name]) {
                                keys.push(name);
                                updates[name] = true;
                            }
                        }
                ...
                this.recordTVarVersion(id, globalTVar);
