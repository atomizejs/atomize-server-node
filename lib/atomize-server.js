...
(function () {
    ...
    Client.prototype = {
        ...
        addToMapping: function (globalTVarId, localTVarId) {
            ...
            this.localGlobal[localTVarId] = globalTVarId;
            this.globalLocal[globalTVarId] = localTVarId;
            return localTVarId;
        },

        toGlobalTVar: function (localTVarId) {
            return globalTVars[this.localGlobal[localTVarId]];
        },

        toGlobalTVarID: function (localTVarId) {
            return this.localGlobal[localTVarId];
        },
        ...
        retry: function (txnLog) {
            ...
            if (ok) {
                ...
                observer = function (globalTVar, future) {
                    ...
                    future.insert(
                        ...
                        function () {
                            if (!fired) {
                                fired = true;
                                self.write(self.createUpdates(updates));
                ...
                this.write(this.createUpdates(updates));
        ...
        createUpdates: function (updates) {
            ...
            while (0 < keys.length) {
                ...
                if (undefined === this.toLocalTVarID(globalTVar.id)) {
                    txnLog.updates[this.addToMapping(globalTVar.id)] = localTVar;
                } else {
                    txnLog.updates[this.toLocalTVarID(globalTVar.id)] = localTVar;
                }
                ...
                for (i = 0; i < names.length; i += 1) {
                    ...
                    if (util.hasOwnProp.call(desc.value, 'tvar')) {
                        ...
                        if (undefined === this.toLocalTVarID(desc.value.tvar)) {
                            keys.push(desc.value.tvar); // push the global id
                            desc.tvar = this.addToMapping(desc.value.tvar);
                        } else {
                            desc.tvar = this.toLocalTVarID(desc.value.tvar);
                        }
                        delete desc.value;
                    } else {
                        localTVar.value[name] = desc;
